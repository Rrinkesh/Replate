...
import { PageHeader, Input, Select, Button, Badge, Card, Modal, EmptyState, LoadingSpinner } from '../../components/common';
...
const BusinessFoodPage = () => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      alert('Failed to delete food listing: ' + (err.response?.data?.message || err.message));
    } finally {
      setIsDeleting(false);
    }
  };

  const TABS = ['All', 'Available', 'Expiring Soon', 'Almost Expired', 'Sold Out', 'Expired'];
        ...
        <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
          ...
          <form onSubmit={handleSearch} className="flex gap-2 w-full md:w-auto">
            ...
            <div className="flex gap-2 flex-1 md:w-64">
              ...
              />
              <Button type="submit" variant="outline" className="px-3" iconLeft={Search}>
              ...
              <tbody className="divide-y divide-charcoal-50">
                {loading ? (
                  ...
                  foodListings.map((item) => {
                    ...
                    return (
                      <tr key={item._id || item.id} className="hover:bg-surface-50 transition-colors group">
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-3">
                            <img
                              src={item.image || 'https://images.unsplash.com/photo-1555396273-367ea4eb4db5?auto=format&fit=crop&w=400&q=80'}
                              alt={item.name}
                              className={`w-10 h-10 rounded-lg object-cover ${isExpired || isSoldOut ? 'opacity-50 grayscale' : ''}`}
                            />
                            <div>
                              <p className={`font-extrabold text-sm ${isExpired || isSoldOut ? 'text-charcoal-500 line-through' : 'text-charcoal-900'}`}>
                                {item.name}
                              </p>
                        ...
                        <td className="px-4 py-3 text-right">
                          <span className={`font-black text-sm ${item.quantity <= 0 ? 'text-rose-600' : 'text-brand-700'}`}>
                            {item.quantity}
                          </span>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-1.5 text-charcoal-600 font-medium">
                            <Clock className={`w-3.5 h-3.5 ${isExpired ? 'text-rose-500' : 'text-amber-600'}`} />
                            {item.expiryTime ? new Date(item.expiryTime).toLocaleString([], {
                        ...
                        <td className="px-4 py-3 text-right">
                          <div className="flex items-center justify-end gap-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                            ...
                            <button
                              onClick={() => navigate('/list-food', { state: { editFood: item } })}
