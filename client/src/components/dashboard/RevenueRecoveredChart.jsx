...
const RevenueRecoveredChart = ({ data = defaultRevenueData, title = 'Revenue Recovered (₹)', className = '' }) => {
  ...
  return (
    <Card variant="default" className={`p-5 ${className}`}>
      ...
      <div className="text-2xl font-black text-charcoal-900 tracking-tight mb-4">
        ₹{data[data.length - 1]?.value.toLocaleString()} <span className="text-xs font-semibold text-charcoal-500">this month</span>
      </div>
      ...
      <div className="h-28 flex items-end justify-between gap-2 pt-2 px-1 border-b border-charcoal-200">
        {data.map((item, idx) => {
          ...
          return (
            <div key={idx} className="flex-1 flex flex-col items-center gap-1 justify-end h-full group">
              ...
              />
              <span className="text-[10px] font-bold text-charcoal-500 mt-1">{item.week}</span>
            </div>
          );
        })}
      </div>
      ...
      <div className="mt-3 flex items-center justify-between text-xs text-charcoal-500 font-medium">
        ...
        <span className="font-bold text-charcoal-900">₹{total.toLocaleString()}</span>
      </div>
    </Card>
  );
};
