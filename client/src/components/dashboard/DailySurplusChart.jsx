...
const DailySurplusChart = ({ data = defaultDailyData, title = 'Daily Surplus Volume', className = '' }) => {
    ...
    <Card variant="default" className={`p-5 ${className}`}>
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-charcoal-500">Recorded surplus (kg) per day</p>
        </div>
        ...
        {data.map((bar, i) => {
          ...
          return (
            <div key={i} className="flex-1 flex flex-col items-center gap-1.5 h-full justify-end group">
              ...
              />
              <span className="text-[11px] font-bold text-charcoal-600 mt-1">{bar.day}</span>
            </div>
          );
        })}
      </div>
