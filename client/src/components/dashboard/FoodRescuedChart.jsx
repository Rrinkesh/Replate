...
const FoodRescuedChart = ({ data = defaultCategoryData, title = 'Food Rescued Distribution', className = '' }) => {
  return (
        ...
        {data.map((item, idx) => (
          <div key={idx} className="flex items-center justify-between text-xs">
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold text-charcoal-800">{item.category}</span>
            </div>
            <span className="font-extrabold text-charcoal-900">{item.percentage}%</span>
          </div>
        ))}
      </div>
    </Card>
