...
const Input = React.forwardRef(({
  ...
  return (
    <div className={`w-full ${containerClassName}`}>
        ...
        {IconRight && (
          <div
            ...
            className={`absolute inset-y-0 right-0 pr-3.5 flex items-center text-charcoal-500 ${
              onRightIconClick ? 'cursor-pointer hover:text-charcoal-800' : 'pointer-events-none'
            }`}
          >
      ...
      {error && (
        <p id={errorId} className="mt-1.5 text-xs font-semibold text-rose-600 flex items-center gap-1">
      ...
      {!error && helperText && (
        <p id={helperId} className="mt-1.5 text-xs text-charcoal-500 font-medium">
          {helperText}
        </p>
      )}
    </div>
