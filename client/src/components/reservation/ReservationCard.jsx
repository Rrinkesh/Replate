...
const ReservationCard = ({
  ...
  const canRecipientCancel =
    isRecipient && ['PENDING', 'CONFIRMED'].includes(reservation.status?.toUpperCase());

  return (
    <Card variant="default" className="shadow-soft-sm hover:shadow-soft-md transition-shadow">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-[11px] text-charcoal-400 font-medium">
                • Reserved {new Date(reservation.reservedAt || reservation.createdAt).toLocaleDateString()}
              </span>
            </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 py-4 text-xs">
        ...
        <div className="p-3 bg-surface-50 rounded-xl border border-charcoal-100">
          <span className="text-charcoal-500 font-medium block mb-0.5">Reserved Quantity:</span>
        ...
        {/* Total Price */}
        <div className="p-3 bg-surface-50 rounded-xl border border-charcoal-100">
          <span className="text-charcoal-500 font-medium block mb-0.5">Recovery Price:</span>
        ...
        {/* Pickup Window */}
        <div className="p-3 bg-surface-50 rounded-xl border border-charcoal-100">
          <span className="text-charcoal-500 font-medium block mb-0.5">Pickup Deadline:</span>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {isBusiness && reservation.status === 'PENDING' && (
            <Button
              ...
              variant="primary"
              disabled={isUpdating}
              onClick={() => onStatusChange && onStatusChange(reservation._id, 'CONFIRMED')}
            >
              Confirm Claim
            </Button>
          )}

          {isBusiness && ['PENDING', 'CONFIRMED'].includes(reservation.status) && (
            <Button
              size="sm"
              variant="outline"
              disabled={isUpdating}
              onClick={() => onStatusChange && onStatusChange(reservation._id, 'READY_FOR_PICKUP')}
            >
          ...
          {isBusiness && reservation.status === 'READY_FOR_PICKUP' && (
            <Button
              ...
              onClick={() => onStatusChange && onStatusChange(reservation._id, 'COMPLETED')}
            >
              Complete Pickup
            </Button>
          )}

          {isBusiness && !['COMPLETED', 'CANCELLED'].includes(reservation.status) && (
            <Button
              size="sm"
              variant="danger"
              disabled={isUpdating}
              onClick={() => onStatusChange && onStatusChange(reservation._id, 'CANCELLED')}
            >
              Cancel
            </Button>
          )}
