...
api.interceptors.request.use(
  async (config) => {
    try {
      ...
      console.warn('Axios Auth Interceptor token fetch warning:', error.message);
      const savedUserStr = localStorage.getItem('replate_current_user');
      if (savedUserStr) {
        try {
          const savedUser = JSON.parse(savedUserStr);
...
api.interceptors.response.use(
  ...
  (error) => {
    ...
    if (status === 401) {
      console.warn('API Unauthorized (401): Authentication session expired or missing token.');
    } else if (status === 403) {
      console.warn('API Forbidden (403): Role permissions insufficient for this action.');
    } else if (status === 404) {
