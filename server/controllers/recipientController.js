...
const getAuthenticatedRecipientUser = async (req, res) => {
  ...
  if (mongoUser.role !== 'RECIPIENT' && mongoUser.role !== 'ADMIN') {
    const err = new Error('Forbidden - Only recipient accounts can access recipient profiles');
    err.statusCode = 403;
    throw err;
  }
  ...
  try {
    ...
    if (!profile) {
      profile = await RecipientProfile.create({
        ...
        organizationName: mongoUser.organizationName || mongoUser.name || 'Recipient NGO Shelter',
        recipientType: 'NGO',
...
const createRecipientProfile = async (req, res, next) => {
  try {
    ...
    const existingProfile = await RecipientProfile.findOne({ userId: mongoUser._id });
    if (existingProfile) {
      res.status(400);
      throw new Error('Recipient profile already exists. Use PUT /api/recipients/profile to update.');
    }

    const { organizationName, recipientType, description, phone, address, city, state, pincode, profileImage } = req.body;
...
const updateRecipientProfile = async (req, res, next) => {
  try {
    ...
    if (!profile) {
      profile = await RecipientProfile.create({
        userId: mongoUser._id,
        organizationName: organizationName || mongoUser.organizationName || mongoUser.name,
        recipientType: recipientType ? recipientType.toUpperCase() : 'NGO',
        description,
        phone,
        address,
