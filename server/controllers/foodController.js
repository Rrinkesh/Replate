...
const createFood = async (req, res, next) => {
  try {
    ...
    if (mongoUser.role !== 'BUSINESS' && mongoUser.role !== 'ADMIN') {
      res.status(403);
      throw new Error('Forbidden - Only business accounts can post surplus food listings');
    }
    ...
    const food = await Food.create({
      ...
      pickupLocation: pickupLocation || mongoUser.location || { address: '', city: 'Noida' },
...
const getFoodListings = async (req, res, next) => {
  try {
    const { search, category, status, minPrice, maxPrice, businessId } = req.query;
    ...
    if (status && status !== 'All') {
      filteredListings = updatedListings.filter((item) => item.status === status);
    } else if (!businessId) {
      // Exclude expired listings by default in public marketplace
      filteredListings = updatedListings.filter((item) => item.status !== 'EXPIRED');
    }
...
const updateFood = async (req, res, next) => {
  try {
    ...
    if (food.businessId.toString() !== mongoUser._id.toString() && mongoUser.role !== 'ADMIN') {
      res.status(403);
      throw new Error('Forbidden - You can only modify your own food listings');
    }

    // Update allowed fields
    const { name, description, category, quantity, quantityUnit, expiryTime, price, originalPrice, status, image } = req.body;
    ...
    food.originalPrice = originalPrice !== undefined ? Number(originalPrice) : food.originalPrice;
...
const getMyFoodListings = async (req, res, next) => {
  try {
    ...
    if (mongoUser.role !== 'BUSINESS' && mongoUser.role !== 'ADMIN') {
      res.status(403);
      throw new Error('Forbidden - Only business accounts can manage their food inventory');
    }
    ...
    if (status && status !== 'All') {
      if (status === 'Available') {
        filteredListings = enhancedListings.filter(item => 
          ['AVAILABLE', 'EXPIRING_SOON', 'ALMOST_EXPIRED'].includes(item.status)
        );
      } else {
        filteredListings = enhancedListings.filter(item => item.status === status.toUpperCase().replace(' ', '_'));
      }
    }
