...
const getAuthenticatedBusinessUser = async (req, res) => {
  ...
  if (mongoUser.role !== 'BUSINESS' && mongoUser.role !== 'ADMIN') {
    const err = new Error('Forbidden - Only business accounts can access business profiles');
    err.statusCode = 403;
    throw err;
  }
  ...
  try {
    ...
    if (!profile) {
      ...
      profile = await BusinessProfile.create({
        ...
        businessName: mongoUser.organizationName || mongoUser.name || 'Commercial Kitchen',
...
const createBusinessProfile = async (req, res, next) => {
  try {
    ...
    const existingProfile = await BusinessProfile.findOne({ userId: mongoUser._id });
    if (existingProfile) {
      res.status(400);
      throw new Error('Business profile already exists. Use PUT /api/businesses/profile to update.');
    }

    const { businessName, businessType, description, phone, address, city, state, pincode, profileImage } = req.body;
...
const updateBusinessProfile = async (req, res, next) => {
  try {
    ...
    if (!profile) {
      profile = await BusinessProfile.create({
        userId: mongoUser._id,
        businessName: businessName || mongoUser.organizationName || mongoUser.name,
        businessType: businessType ? businessType.toUpperCase() : 'RESTAURANT',
        description,
        phone,
        address,
