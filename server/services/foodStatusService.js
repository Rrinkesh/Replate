  ...
  try {
    ...
    for (const food of activeListings) {
      ...
      if (computedStatus !== food.status) {
        ...
        } else if (computedStatus === 'EXPIRING_SOON' && prevStatus === 'AVAILABLE') {
          await createNotificationHelper({
            userId: food.businessId,
            type: 'FOOD_EXPIRING',
    ...
    if (updatedCount > 0) {
      console.log(`[ExpiryJob] Updated status for ${updatedCount} food listings at ${now.toLocaleTimeString()}`);
    }

    return updatedCount;
  } catch (error) {
    console.error('[ExpiryJob Error] Failed to update food statuses:', error.message);
    return 0;
  }
};
